...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有会员表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存会员表")
@ApiOperation("保存会员表")
...
@SysLog(MODULE = "sys", REMARK = "更新会员表")
@ApiOperation("更新会员表")
...
@SysLog(MODULE = "sys", REMARK = "删除会员表")
@ApiOperation("删除会员表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("会员表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给会员表分配会员表")
@ApiOperation("查询会员表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("会员表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("会员表id");
        }
