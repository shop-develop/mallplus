...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有会员与产品分类关系表（用户喜欢的分类）列表")
...
@SysLog(MODULE = "sys", REMARK = "保存会员与产品分类关系表（用户喜欢的分类）")
...
@SysLog(MODULE = "sys", REMARK = "更新会员与产品分类关系表（用户喜欢的分类）")
...
@SysLog(MODULE = "sys", REMARK = "删除会员与产品分类关系表（用户喜欢的分类）")
@ApiOperation("删除会员与产品分类关系表（用户喜欢的分类）")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("会员与产品分类关系表（用户喜欢的分类）id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给会员与产品分类关系表（用户喜欢的分类）分配会员与产品分类关系表（用户喜欢的分类）")
@ApiOperation("查询会员与产品分类关系表（用户喜欢的分类）明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("会员与产品分类关系表（用户喜欢的分类）id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("会员与产品分类关系表（用户喜欢的分类）id");
        }
