...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有会员任务表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存会员任务表")
...
@SysLog(MODULE = "sys", REMARK = "更新会员任务表")
...
@SysLog(MODULE = "sys", REMARK = "删除会员任务表")
@ApiOperation("删除会员任务表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("会员任务表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给会员任务表分配会员任务表")
@ApiOperation("查询会员任务表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("会员任务表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("会员任务表id");
        }
