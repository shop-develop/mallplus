...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有积分变化历史记录表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存积分变化历史记录表")
...
@SysLog(MODULE = "sys", REMARK = "更新积分变化历史记录表")
...
@SysLog(MODULE = "sys", REMARK = "删除积分变化历史记录表")
@ApiOperation("删除积分变化历史记录表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("积分变化历史记录表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给积分变化历史记录表分配积分变化历史记录表")
@ApiOperation("查询积分变化历史记录表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("积分变化历史记录表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("积分变化历史记录表id");
        }
