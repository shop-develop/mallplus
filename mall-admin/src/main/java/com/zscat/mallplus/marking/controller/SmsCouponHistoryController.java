...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有优惠券使用、领取历史表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存优惠券使用、领取历史表")
...
@SysLog(MODULE = "sys", REMARK = "更新优惠券使用、领取历史表")
...
@SysLog(MODULE = "sys", REMARK = "删除优惠券使用、领取历史表")
@ApiOperation("删除优惠券使用、领取历史表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("优惠券使用、领取历史表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给优惠券使用、领取历史表分配优惠券使用、领取历史表")
@ApiOperation("查询优惠券使用、领取历史表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("优惠券使用、领取历史表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("优惠券使用、领取历史表id");
        }
