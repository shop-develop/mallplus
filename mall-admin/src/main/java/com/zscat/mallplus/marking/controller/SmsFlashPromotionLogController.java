...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有限时购通知记录列表")
...
@SysLog(MODULE = "sys", REMARK = "保存限时购通知记录")
...
@SysLog(MODULE = "sys", REMARK = "更新限时购通知记录")
...
@SysLog(MODULE = "sys", REMARK = "删除限时购通知记录")
@ApiOperation("删除限时购通知记录")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("限时购通知记录id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给限时购通知记录分配限时购通知记录")
@ApiOperation("查询限时购通知记录明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("限时购通知记录id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("限时购通知记录id");
        }
