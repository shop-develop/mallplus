...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有限时购场次表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存限时购场次表")
...
@SysLog(MODULE = "sys", REMARK = "更新限时购场次表")
...
@SysLog(MODULE = "sys", REMARK = "删除限时购场次表")
@ApiOperation("删除限时购场次表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("限时购场次表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给限时购场次表分配限时购场次表")
@ApiOperation("查询限时购场次表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("限时购场次表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("限时购场次表id");
        }
