...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有红包列表")
...
@SysLog(MODULE = "sys", REMARK = "删除红包")
@ApiOperation("删除红包")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("红包id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给红包分配红包")
@ApiOperation("查询红包明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("红包id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("红包id");
        }
