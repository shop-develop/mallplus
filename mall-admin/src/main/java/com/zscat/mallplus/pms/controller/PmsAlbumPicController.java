...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有画册图片表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存画册图片表")
...
@SysLog(MODULE = "sys", REMARK = "更新画册图片表")
...
@SysLog(MODULE = "sys", REMARK = "删除画册图片表")
@ApiOperation("删除画册图片表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("画册图片表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给画册图片表分配画册图片表")
@ApiOperation("查询画册图片表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("画册图片表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("画册图片表id");
        }
