...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有产品阶梯价格表(只针对同商品)列表")
...
@SysLog(MODULE = "sys", REMARK = "保存产品阶梯价格表(只针对同商品)")
...
@SysLog(MODULE = "sys", REMARK = "更新产品阶梯价格表(只针对同商品)")
...
@SysLog(MODULE = "sys", REMARK = "删除产品阶梯价格表(只针对同商品)")
@ApiOperation("删除产品阶梯价格表(只针对同商品)")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("产品阶梯价格表(只针对同商品)id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给产品阶梯价格表(只针对同商品)分配产品阶梯价格表(只针对同商品)")
@ApiOperation("查询产品阶梯价格表(只针对同商品)明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("产品阶梯价格表(只针对同商品)id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("产品阶梯价格表(只针对同商品)id");
        }
