...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）列表")
...
@SysLog(MODULE = "sys", REMARK = "保存产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）")
...
@SysLog(MODULE = "sys", REMARK = "更新产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）")
...
@SysLog(MODULE = "sys", REMARK = "删除产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）")
@ApiOperation("删除产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）分配产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）")
@ApiOperation("查询产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类）id");
        }
